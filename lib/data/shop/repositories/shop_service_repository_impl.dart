...
class ShopServiceRepositoryImpl extends ShopServiceRepository {
  ...
  @override
  Future<Either<Failure, ServiceResponseEntity>> getService() async {
    try {
      var returnedData = await remoteDataSource.getService();
  ...
  Future<Either<Failure, List<BookingEntity>>> getBooking() async {
    try {
      var returnedData = await remoteDataSource.getBooking();
      return Right(returnedData);
    } on ServerFailure {
      return Left(ServerFailure(''));
    } on AuthenticationFailure {
